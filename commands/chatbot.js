...
if (!GROQ_API_KEY) {
  console.error('[CHATBOT] FATAL: GROQ_API_KEY environment variable not set!');
  process.exit(1);
}

// ─── Developer Info (env se lo) ──────────────
const DEV_NAME    = process.env.DEV_NAME    || 'UZAIR MD';
...
const getBotName = (botNum) => botNames.get(botNum.replace(/[^0-9]/g,'')) || DEV_NAME;
...
${toSmallCaps('developer name')}: ${DEV_NAME}
${toSmallCaps('contact')}: ${DEV_CONTACT}
${toSmallCaps('channel')}: ${DEV_CHANNEL}`;
...
const groqReply = async (userMessage, botName, topic) => {
  ...
  try {
    ...
    // Sirf message log karo — stack trace nahi (sensitive info leak se bachao)
